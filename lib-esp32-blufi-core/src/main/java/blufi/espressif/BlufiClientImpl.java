...
class BlufiClientImpl implements BlufiParameter {
    ...
    BlufiClientImpl(BlufiClient client, Context context, BluetoothDevice device) {
        ...
        //create the gatt callback
        mInnerGattCallback = new InnerGattCallback();
        //create the sequence for read and write BLE commands
    ...
    //set the gatt callback which is an external gatt callback
    void setGattCallback(BluetoothGattCallback callback) {
        mUserGattCallback = callback;
    }

    //set the blufi callback which is an external blufi callback
    ...
    //clean all the resources that have been allocated
    ...
    //set the gatt write timeout
    void setGattWriteTimeout(long timeout) {
        mWriteTimeout = timeout;
    }

    //set the package length limit, if < 20, will use 20, otherwise use the lengthLimit parameter
    ...
    //request the device version from the device
    ...
    //request the device status from device
    ...
    //negotiate the security with the device
    ...
    //request the device to scan the wifi list
    ...
    //post the custom data to the device
    ...
    //request the device to close the connection
    ...
    private String toHex(byte[] bytes) {
        ...
        for (byte b : bytes) {
            ...
            if (str.length() == 1) {
                sb.append("0"); //append 0 if the length of the str is 1
            }
            sb.append(str);
        }
        return sb.toString();
    ...
    //convert the hex str to byte array
    ...
    private boolean gattWrite(byte[] data) throws InterruptedException {
        ...
        //set the data as the value of the BLE write characteristic
        mWriteChar.setValue(data);
        //write the chars, send the data/characteristic from the mobile phone to the device
        mGatt.writeCharacteristic(mWriteChar);
        Boolean result;
        if (mWriteTimeout > 0) {
            //if the timeout > 0, get the write result with the specific timeout
        ...
        //write the data/characteristic succeed or not
        return result != null && result;
    }

    //receive the acknowledge and compare the expectAck/sequence with the received ack
    ...
    //post the data considering whether encrypt the data, whether add the checksum and requireAck
    ...
    //post the package without the data
    private boolean postNonData(boolean encrypt, boolean checksum, boolean requireAck, int type)
        ...
        //get the data to write to gatt
        byte[] postBytes = getPostBytes(type, encrypt, checksum, requireAck, false, sequence, null);
        //write to gatt which get from the device
        boolean posted = gattWrite(postBytes);
        //check the post result and check whether the ack is needed and the ack is received and matched
        return posted && (!requireAck || receiveAck(sequence));
    }

    //post the package with the data
    private boolean postContainData(boolean encrypt, boolean checksum, boolean requireAck, int type, byte[] data)
        ...
        //there are 4 bytes at the head of the package
        int postDataLengthLimit = pkgLengthLimit - PACKAGE_HEADER_LENGTH;
        postDataLengthLimit -= 2; // if fragment the package, two bytes in data will be used as the total data length
        if (checksum) {
        ...
        while (true) {
            //read the data from dataIS to the dataBuf
            int read = dataIS.read(dataBuf, 0, dataBuf.length);
            if (read == -1) {
                break;
            }
            //write the data from Buf to the output stream
            dataContent.write(dataBuf, 0, read);
            //if there is still has data in the dataIS and the available data size <= 2, then it means there is no need to fragment the data
            if (dataIS.available() > 0 && dataIS.available() <= 2) {
                read = dataIS.read(dataBuf, 0, dataIS.available());
                dataContent.write(dataBuf, 0, read);
            }
            //if there is still has available data in the dataIS then it means there is no need to fragment the data
            boolean frag = dataIS.available() > 0;
            int sequence = generateSendSequence();
            if (frag) {
                //if frag, calculate the total length
                int totalLen = dataContent.size() + dataIS.available();
                //get the already read data to tempData
                byte[] tempData = dataContent.toByteArray();
                dataContent.reset();
                //write the two bytes total length to the dataContent
                dataContent.write(totalLen & 0xff);
                dataContent.write(totalLen >> 8 & 0xff);
                //write the tempData again
            ...
            //if data is fragmented, first to check whether the ack is received or not, if not, abort the gatt write
            if (frag) {
    ...
    //get the post bytes to be send to the device
    private byte[] getPostBytes(int type, boolean encrypt, boolean checksum, boolean requireAck, boolean hasFrag, int sequence, byte[] data) {
        //set the byte output stream
        ByteArrayOutputStream byteOS = new ByteArrayOutputStream();

        int dataLength = data == null ? 0 : data.length;
        //DIRECTION_OUTPUT means from the mobile phone to the ESP device.
        ...
        //one byte for sequence
        byteOS.write(sequence);
        //write the dataLength as the 4th byte
        byteOS.write(dataLength);

        byte[] checksumBytes = null;
        if (checksum) {
            //use the "sequence" and "dataLength" to generate the checkSum
            byte[] willCheckBytes = new byte[]{(byte) sequence, (byte) dataLength};
            int crc = BlufiCRC.calcCRC(0, willCheckBytes);
            //if there is data to be transmit, calculate the checkSum based on the data
            if (dataLength > 0) {
                crc = BlufiCRC.calcCRC(crc, data);
            }
            //get the two bytes check sum
            checksumBytes = new byte[]{(byte) (crc & 0xff), (byte) (crc >> 8 & 0xff)};
        }

        //in case need to encrypt the data
        if (encrypt && data != null && data.length > 0) {
            //use the sequence to generate the IV and use AES to encrypt the data
            BlufiAES aes = new BlufiAES(mAESKey, AES_TRANSFORMATION, generateAESIV(sequence));
            data = aes.encrypt(data);
        }
        //write the data, need to make sure the length of the encrypted data is the same as the unencrypted data
        if (data != null) {
            byteOS.write(data, 0, data.length);
        }

        //write the last two check sum bytes
...
//            int totalLen = dataBytes[0] | (dataBytes[1] << 8);
    ...
    private void __negotiateSecurity() {
        ...
        try {
            //generate the secret key from the devicePublicKey
            ...
            //generate the AESKey for all the following data encryption
    ...
    private class InnerGattCallback extends BluetoothGattCallback {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                //if the newState is STATE_CONNECTED
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        //request the high priority
            ...
            //propagate the event to the user callback
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                //get the service with the service UUID
                ...
                if (service != null) {
                    ...
                    //get the notification characteristic from the service
                    notifyChar = service.getCharacteristic(BlufiParameter.UUID_NOTIFICATION_CHARACTERISTIC);
                    if (notifyChar != null) {
                        //enable the notification characteristic from the device
            ...
            if (mUserBlufiCallback != null) {
                ...
                //get the notification descriptor from the notification characteristic
                ...
                if (service != null && writeChar != null && notifyChar != null && notifyDesc != null) {
                    ...
                    //set the ENABLE_NOTIFICATION_VALUE to the notification descriptor
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            //if the characteristic is notification characteristic
        ...
        //when the characteristic is wrote and receive the callback, then add the result to the queue
