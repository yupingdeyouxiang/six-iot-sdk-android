...
class FrameCtrlData {
    ...
    static int getFrameCTRLValue(boolean encrypted, boolean checksum, int direction, boolean requireAck, boolean frag) {
        ...
        if (direction == BlufiParameter.DIRECTION_INPUT) { //if this bit is 1, then means the direction is from device to the mobile phone
            frame = frame | (1 << FRAME_CTRL_POSITION_DATA_DIRECTION);
        }
        if (requireAck) { //require the ACK
            frame = frame | (1 << FRAME_CTRL_POSITION_REQUIRE_ACK);
        }
        if (frag) { //meaning the frame is fragmented
